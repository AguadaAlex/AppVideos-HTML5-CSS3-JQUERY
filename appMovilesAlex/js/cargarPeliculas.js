...
function busquedaPorNombre(elIdgege){
        ...
        $.get(url,function(datos){
            ...
            $.each(datos.results,function(index,obj){
                if(obj.poster_path!=null){
                    console.log('ahora si estoy aca');
...
function busquedaPorGenero(elIdgenero){
    var descrip;
    var totalResultados;
    var totalPaginas;
    var pag;
    ...
    if(getId===null){
    url =`https://api.themoviedb.org/3/discover/movie?api_key=e2686a810371064534c95f86b3f8ed0a&language=es-ES&sort_by=primary_release_date.desc&page=1&with_genres=${geneId}`;
    }else{
        url =`https://api.themoviedb.org/3/discover/movie?api_key=e2686a810371064534c95f86b3f8ed0a&language=es-ES&sort_by=primary_release_date.desc&page=1&with_genres=${geneId}&page=${getId}`;
    }
    console.log(url);
    $.get(url,function(datos){
        console.log(datos.results);
          ...
          if(totalResultados<=10){
            $(".paginacion").html(`
            <ul class="muje">
            <li><a href="index2.html?geneId=${geneId}&Id=1" class="active">${pag}</a></li>
          </ul>
            `);
        }
        else
            ...
            if(pag<totalPaginas){
                if(pag>1){
                    if(pag+2>totalPaginas){
                        $(".paginacion").html(`
                        <ul class="muje">
                        <li><a href="index2.html?geneId=${geneId}&Id=${pag-1}">Prev</a></li>
                        <li><a href="index2.html?geneId=${geneId}&Id=${pag}" class="active">${pag}</a></li>
                        <li><a href="index2.html?geneId=${geneId}&Id=${pag+1}">${pag + 1}</a></li>
                    ...
                    <li><a href="index2.html?geneId=${geneId}&Id=${pag}" class="active">${pag}</a></li>
                    <li><a href="index2.html?geneId=${geneId}&Id=${pag+1}">${pag + 1}</a></li>
                    <li><a href="index2.html?geneId=${geneId}&Id=${pag+2}">${pag + 2}</a></li>
                    <li><a href="index2.html?geneId=${geneId}&Id=${pag+1}">next</a></li>
                </ul>
                `);
                }
                else{
...
function populares(){
    ...
    if(getId===null){
            ...
            url ='https://api.themoviedb.org/3/movie/upcoming?api_key=e2686a810371064534c95f86b3f8ed0a&language=es-MX&total_pages';
    }
    else{
    url =`https://api.themoviedb.org/3/movie/upcoming?api_key=e2686a810371064534c95f86b3f8ed0a&language=es-MX&page=${getId}`;
    }
        ...
        aux=0;
            $.each(datos.results,function(index,obj){
                ...
                if(totalResultados<=10){
                    $(".paginacion").html(`
                    <ul class="muje">
                    <li><a href="index2.html?Id=1" class="active">${pag}</a></li>
                  </ul>
                    `);
                }
                else
                    ...
                    if(pag<totalPaginas){
                        if(pag>1){
                            if(pag+2>totalPaginas){
                                $(".paginacion").html(`
                                <ul class="muje">
                                <li><a href="index2.html?Id=${pag-1}">Prev</a></li>
                                <li><a href="index2.html?Id=${pag}" class="active">${pag}</a></li>
                                <li><a href="index2.html?Id=${pag+1}">${pag + 1}</a></li>
                            ...
                            <li><a href="index2.html?Id=${pag}" class="active">${pag}</a></li>
                            <li><a href="index2.html?Id=${pag+1}">${pag + 1}</a></li>
                            <li><a href="index2.html?Id=${pag+2}">${pag + 2}</a></li>
                            <li><a href="index2.html?Id=${pag+1}">next</a></li>
                        </ul>
                        `);
                        }
                        else{
...
// CARGAR PELICULAS DINAMICAMENTE-------------------------------------------------------
$(document).ready(function(){

try{
populares();
// BUSQUEDA POR NOMBRE
 $("#ultimaB li").on("click",function(){
     // OBTENER ID DE PELICULA
     var elIdgege = $(this).attr('id');
    console.log(elIdgege);
    busquedaPorNombre(elIdgege);
    
 });
// BUSQUEDA POR GENERO 
$("#generoP li").on("click",function(){
    // OBTENER ID DE PELICULA
    console.log("probando");
    var elIdgenero = $(this).children('li').attr('id');
    // var elNombreGenero=$(this).children('a').text();
    console.log(elIdgenero);
    // console.log(elNombreGenero);
    busquedaPorGenero(elIdgenero);
...
console.log(getId);
 ...
 $(document).on('click', '#generoP > li', function () {
     // OBTENER ID DE PELICULA
     console.log("probando");
     var elIdgenero = $(this).attr('id');
     // var elNombreGenero=$(this).children('a').text();
     console.log(elIdgenero);
     // console.log(elNombreGenero);
     busquedaPorGenero(elIdgenero);
